...
export default function Allproducts() {
    ...
    return (
        <>
                        <h2 className="text-2xl font-bold tracking-tight text-gray-900">All products Collection</h2>

            <div className="flex flex-wrap  gap-5 justify-center md:max-w-5xl lg:max-w-full relative mt-16">
                {data ? data.map((item) => {
                    return (
                        <a href={`/description/${item.id}`} key={item.id} className='p-2 w-1/3 md:w-1/4 lg:w-1/5 md:max-w-5xl bg-slate-200 flex flex-col rounded-md'>
